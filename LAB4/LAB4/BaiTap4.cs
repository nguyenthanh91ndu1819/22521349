...
namespace LAB4
{
    public partial class BaiTap4 : Form
    {
        ...
        private void LoadMovieDataFromHTML(string filePath)
        {
            try
            {
                ...
                if (movieNodes != null)
                {
                    foreach (var movieNode in movieNodes)
                    {
                        ...
                        if (bannerNode != null && titleNode != null && linkNode != null)
                        {
                            ...
                            DisplayMovieInfoInPanel(bannerUrl, movieTitle, movieUrl);
                        }
                    }
                }
                else
        ...
        private void DisplayMovieInfoInPanel(string bannerUrl, string movieTitle, string movieUrl)
        {
            try
            {
                ...
                pictureBox.Click += (sender, e) => { OpenWebViewForm(movieUrl); };
        ...
        private void OpenWebViewForm(string movieUrl)
        {
            WebViewBai4 webViewForm = new WebViewBai4(movieUrl);
            webViewForm.Show();
        }
        ...
        private async Task CreateAndSaveMovieDataToJson(string htmlFilePath, string jsonFilePath)
        {
            try
            {
                // Extract movie names from HTML file
                List<string> movieNames = await ExtractMovieNamesFromHTML(htmlFilePath);

                // Create random price and screening room for each movie
                Random random = new Random();
                List<Phim> phimList = new List<Phim>();
                foreach (string movieName in movieNames)
                {
                    int giaVeChuan = random.Next(90000, 150000); // Random price between 90000 and 150000 VND
                    List<int> phongChieu = new List<int> { random.Next(1, 11), random.Next(1, 11) }; // Random room numbers between 1 and 10
